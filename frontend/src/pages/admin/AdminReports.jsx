      ...
      <table className={`${ADMIN_TABLE_CLASS} reports-summary-skeleton-table`} aria-hidden="true">
        ...
        <tbody>
          {Array.from({ length: rows }, (_, rowIndex) => (
            <tr key={`admin-report-skeleton-row-${rowIndex}`}>
  ...
  return (
    ...
    >
      ...
      <SectionCard title="Report Filters" className="report-card report-filters-card">
        {filtersLoading ? <FilterSkeleton fields={8} /> : <div className="report-filter-grid report-filter-grid-admin grid grid-cols-4 gap-3.5 max-[992px]:grid-cols-2 max-[640px]:grid-cols-1">
          ...
          <label className="field-label flex flex-col gap-2"><span className="text-[0.9rem] font-semibold text-[#475569]">Subject</span><select value={filters.subject} onChange={(e) => setFilters((p) => ({ ...p, subject: e.target.value }))} className="w-full min-h-11 rounded-xl border border-[#cbd5e1] bg-white p-[10px_12px] text-[#1f2937] dark:border-[#334155] dark:bg-[#0f172a] dark:text-[#e5e7eb]"><option value="">All subjects (combined)</option>{options.subject_options.map((s) => <option key={s} value={s}>{s}</option>)}</select></label>
          <label className="field-label flex flex-col gap-2"><span className="text-[0.9rem] font-semibold text-[#475569]">Faculty</span><select value={filters.faculty_email} onChange={(e) => setFilters((p) => ({ ...p, faculty_email: e.target.value }))} className="w-full min-h-11 rounded-xl border border-[#cbd5e1] bg-white p-[10px_12px] text-[#1f2937] dark:border-[#334155] dark:bg-[#0f172a] dark:text-[#e5e7eb]"><option value="">All faculty</option>{options.faculty_options.map((f) => <option key={f.email} value={f.email}>{f.name} ({f.email})</option>)}</select></label>
